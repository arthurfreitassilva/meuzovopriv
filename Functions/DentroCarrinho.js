...
async function DentroCarrinhoEfiBank(client, interaction) {
    ...
    await interaction.message.edit({ content: `${Emojis.get(`loading_dreamapps`)} Aguarde...`, ephemeral: true, components: [], embeds: [] }).then(async tt => {
        try {
            ...
            tt.edit({ content: `${Emojis.get(`loading_dreamapps`)} Espere só mais um pouco...`, ephemeral: true, components: [], embeds: [] })
        ...
        await mercadopago.payment.create(payment_data)
            .then(async function (data) {
                ...
                const mandanopvdocara = new EmbedBuilder()
                    ...
                    .setDescription(`\`\📡\` Seu pedido foi criado e agora está aguardando a confirmação do pagamento`)
                    .addFields(
                        { name: `\`\🕵️‍♂️\` **Detalhes**`, value: `\`${yy.quantidadeselecionada}x ${yy.infos.produto} - ${yy.infos.campo} | R$ ${Number(valor).toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}\`` },
...
async function DentroCarrinho2(interaction) {
    ...
    if (yd.quantidadeselecionada > gggaaa.condicao?.valormaximo) return interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} Você não pode comprar mais de \`${gggaaa.condicao.valormaximo}x ${yd.infos.produto} - ${yd.infos.campo}\``, ephemeral: true })
    if (yd.quantidadeselecionada < gggaaa.condicao?.valorminimo) return interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} Você não pode comprar mais de \`${gggaaa.condicao.valorminimo}x ${yd.infos.produto} - ${yd.infos.campo}\``, ephemeral: true })
    interaction.deferUpdate()
