    ...
    run: async (interaction, client) => {
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId === "definirMensagem") {
                ...
                if (imagemContent && !imagemValida) {
                    return interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} | URL da imagem inválida. Por favor, insira uma URL de imagem válida.`, ephemeral: true });
                }
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId === "adicionarbotao2") {
                ...
                if (!emojivalido24) {
                    await interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} | O __emoji__ fornecido é inválido. Por favor, use um emoji válido.`, ephemeral: true });
                    return;
                }
        
                if (emojiNoServer24) {
                    await interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} | O __emoji__ fornecido não está disponível no servidor. Por favor, escolha um emoji que está no servidor.`, ephemeral: true });
                    return;
                }
        ...
        if (customId === "previecontent") {
            ...
            if (!mensagem) {
                await interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} | A __mensagem__ não está configurada. Por favor, configure a mensagem antes de enviá-la.`, ephemeral: true });
                return;
            }
