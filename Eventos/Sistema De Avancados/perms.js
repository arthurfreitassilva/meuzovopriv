    ...
    run: async (interaction, client) => {
        ...
        if (customId === 'perm_add' || customId === 'perm_remove') {
            if (owner !== interaction.user.id) {
                return interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} Você não tem permissão para usar este comando.`, ephemeral: true });
            }
        ...
        if (interaction.isModalSubmit()) {
            if (interaction.customId === 'perm_add_modal') {
                ...
                try {
                    ...
                    return interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} O arquivo de permissões não pôde ser carregado.`, ephemeral: true });
                }
                ...
                if (!perms[userId]) {
                    ...
                    try {
                        ...
                        return interaction.reply({ content: `${Emojis.get(`positive_dream`)} O usuário com ID \`${userId}\` foi adicionado à lista de permissões do BOT.**`, ephemeral: true });
                    } catch (error) {
                        console.error("Erro ao salvar o arquivo de permissões:", error);
                        return interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} Houve um erro ao salvar o arquivo de permissões.`, ephemeral: true });
                    }
                } else {
                    return interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} O usuário já possui permissão no BOT.**`, ephemeral: true });
                }
            }
            ...
            if (interaction.customId === 'perm_remove_modal') {
                ...
                if (!perms[userId]) {
                    return interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} O usuário com ID \`${userId}\` não está na lista de permissões do BOT.**`, ephemeral: true });
                }

                delete perms[userId];
                try {
                    fs.writeFileSync(permsarguivo24, JSON.stringify(perms, null, 2));
                    return interaction.reply({ content: `${Emojis.get(`positive_dream`)} O usuário com ID \`${userId}\` foi removido da lista de permissões do BOT.**`, ephemeral: true });
                } catch (error) {
                    console.error("Erro ao salvar o arquivo de permissões:", error);
                    return interaction.reply({ content: `${Emojis.get(`negative_dreamm67`)} Houve um erro ao salvar o arquivo de permissões.`, ephemeral: true });
                }
