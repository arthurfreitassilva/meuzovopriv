    ...
    run: async (interaction, client) => {
        ...
        if (customId === "create_emojis") {
            ...
            try {
                ...
                await interaction.editReply({ content: `${Emojis.get(`positive_dream`)} Emojis adicionados com sucesso neste servidor! Lembre-se de reiniciar o bot para garantir que as alterações entrem em vigor.`, ephemeral: true });
            } catch (error) {
                console.error("Erro ao criar emojis:", error);
                await interaction.editReply({ content: `${Emojis.get(`negative_dreamm67`)} Ocorreu um erro ao adicionar os emojis.`, ephemeral: true });
            }
        }
    }
};
